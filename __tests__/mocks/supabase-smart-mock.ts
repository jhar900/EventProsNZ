// Smart Supabase mock that handles method chaining properly
export const createSmartSupabaseMock = () => {
  const createMockQuery = () => {
    ...
    // Override methods to return promises when they're the final call
    const originalUpdate = mockQuery.update;
    const originalInsert = mockQuery.insert;
    const originalDelete = mockQuery.delete;
    const originalSelect = mockQuery.select;

    // Make update return a promise when called without further chaining
    mockQuery.update = jest.fn().mockImplementation((...args) => {
      const result = originalUpdate.apply(mockQuery, args);
      // If this is the final call in a chain, return a promise
      if (args.length > 0) {
    ...
    // Make insert return a promise when called without further chaining
    mockQuery.insert = jest.fn().mockImplementation((...args) => {
      const result = originalInsert.apply(mockQuery, args);
      // If this is the final call in a chain, return a promise
      if (args.length > 0) {
    ...
    // Make delete return a promise when called without further chaining
    mockQuery.delete = jest.fn().mockImplementation((...args) => {
      const result = originalDelete.apply(mockQuery, args);
      // If this is the final call in a chain, return a promise
      if (args.length > 0) {
        return Promise.resolve({ data: null, error: null });
      }
    ...
    // Make select return a promise when called without further chaining
    mockQuery.select = jest.fn().mockImplementation((...args) => {
      const result = originalSelect.apply(mockQuery, args);
      // If this is the final call in a chain, return a promise
      if (args.length > 0) {
        return Promise.resolve({ data: [{ id: 'test-id' }], error: null });
      }
