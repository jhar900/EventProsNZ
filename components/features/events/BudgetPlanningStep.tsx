...
export function BudgetPlanningStep() {
  ...
  // Initialize breakdown with service requirements when they change or total budget is set
  useEffect(() => {
    if (serviceRequirements.length > 0 && totalBudget > 0) {
      const newBreakdown: {
      ...
      // Add each service requirement to the breakdown
      serviceRequirements.forEach(req => {
        // Use a unique key based on category and type
        const key = `${req.category}_${req.type}`
          .replace(/\s+/g, '_')
          .toLowerCase();

        // If this item doesn't exist in breakdown yet, initialize it
        if (!newBreakdown[key]) {
          // Use estimatedBudget if provided, otherwise calculate from recommendations
          let amount = req.estimatedBudget || 0;

          // If no estimated budget, try to get from recommendations
          if (amount === 0 && recommendations.length > 0) {
            const rec = recommendations.find(r => r.category === req.category);
            if (rec) {
              amount = rec.suggestedAmount;
            }
          }

          // If still no amount, allocate a portion based on total budget
          if (amount === 0) {
            // Allocate equally among all requirements
            amount = Math.round(totalBudget / serviceRequirements.length);
          }
          ...
          // Update percentage for existing items when total budget changes
          const percentage =
            totalBudget > 0
              ? (newBreakdown[key].amount / totalBudget) * 100
              : 0;
          newBreakdown[key] = {
            ...newBreakdown[key],
            percentage: Math.round(percentage * 100) / 100,
      ...
      // Remove items from breakdown that are no longer in service requirements
      Object.keys(newBreakdown).forEach(key => {
        const [category, ...typeParts] = key.split('_');
        const type = typeParts.join('_');
        const exists = serviceRequirements.some(
          req =>
            req.category.toLowerCase() === category.toLowerCase() &&
            req.type.toLowerCase().replace(/\s+/g, '_') === type.toLowerCase()
        );
        if (!exists) {
      ...
      // Clear breakdown if no service requirements
      setBreakdown({});
    }
  }, [serviceRequirements, totalBudget, recommendations]);
  ...
  const handleTotalBudgetChange = (value: string) => {
    ...
    // The breakdown will be auto-generated by the useEffect hook
    // based on service requirements, so we don't need to set it here
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {Object.entries(breakdown).map(([key, item]) => {
              // Find the corresponding service requirement to get display name
              const [categoryPart, ...typeParts] = key.split('_');
              const typePart = typeParts.join('_');
              const requirement = serviceRequirements.find(
                req =>
                  req.category.toLowerCase() === categoryPart.toLowerCase() &&
                  req.type.toLowerCase().replace(/\s+/g, '_') ===
                    typePart.toLowerCase()
              );

              // Format display name: use service type if available, otherwise format the key
              const displayName = requirement
                ? `${requirement.type} (${requirement.category})`
                : key
                    .split('_')
