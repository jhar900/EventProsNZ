...
export class FileSecurityScanner {
  ...
  async scanFile(
    ...
    try {
      ...
      if (result.fileInfo.size > this.config.maxFileSize) {
        result.errors.push(
          `File size ${result.fileInfo.size} exceeds maximum allowed size ${this.config.maxFileSize}`
        );
      ...
      if (!this.config.allowedTypes.includes(result.fileInfo.mimeType)) {
        result.errors.push(
          `File type ${result.fileInfo.mimeType} is not allowed`
        );
        result.isValid = false;
      }
      ...
      if (
        !this.config.allowedExtensions.includes(
          result.fileInfo.extension.toLowerCase()
        )
      ) {
        result.errors.push(
          `File extension ${result.fileInfo.extension} is not allowed`
        );
        result.isValid = false;
      }
      ...
      if (
        !this.validateMagicNumber(
          result.fileInfo.magicNumber,
          result.fileInfo.extension
        )
      ) {
        result.errors.push(
          "File content does not match file extension (possible spoofing)"
        );
        result.isValid = false;
      }
      ...
      // Security scan
      if (result.isValid) {
        result.securityScan = await this.performSecurityScan(
          fileBuffer,
          filename
        );
  ...
  generateSecureFilename(originalName: string, fileHash: string): string {
    ...
    return `${fileHash.substring(
      0,
      16
    )}_${timestamp}_${randomSuffix}.${extension}`;
  }
    ...
    try {
      ...
      for (const signature of executableSignatures) {
        if (content.includes(signature)) {
          threats.push(
            `Suspicious executable signature detected: ${signature}`
          );
        }
      }
      ...
      if (fileBuffer.length > 1024 * 1024) {
        ...
        if (textRatio > 0.1) {
          // More than 10% text content
          threats.push(
            "Suspicious file: high text content ratio in binary file"
          );
        }
      }
    } catch (error) {
  ...
  private async performCloudScan(
    fileBuffer: Buffer,
    filename: string
  ): Promise<{ threats: string[] }> {
    const threats: string[] = [];
