-- Feature Request System Migration (Idempotent Version)
...
CREATE TABLE IF NOT EXISTS feature_request_tag_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_feature_requests_category_id ON feature_requests(category_id);
...
CREATE INDEX IF NOT EXISTS idx_feature_requests_is_featured ON feature_requests(is_featured);
...
CREATE INDEX IF NOT EXISTS idx_feature_request_tag_assignments_feature_request_id ON feature_request_tag_assignments(feature_request_id);
CREATE INDEX IF NOT EXISTS idx_feature_request_tag_assignments_tag_id ON feature_request_tag_assignments(tag_id);

CREATE INDEX IF NOT EXISTS idx_feature_request_status_history_feature_request_id ON feature_request_status_history(feature_request_id);
...
DROP POLICY IF EXISTS "Users can view public feature requests" ON feature_requests;
...
DROP POLICY IF EXISTS "Users can view all votes" ON feature_request_votes;
DROP POLICY IF EXISTS "Users can vote on feature requests" ON feature_request_votes;
DROP POLICY IF EXISTS "Users can update their own votes" ON feature_request_votes;
...
DROP POLICY IF EXISTS "Users can view all comments" ON feature_request_comments;
DROP POLICY IF EXISTS "Users can insert comments" ON feature_request_comments;
DROP POLICY IF EXISTS "Users can update their own comments" ON feature_request_comments;
DROP POLICY IF EXISTS "Users can delete their own comments" ON feature_request_comments;
...
DROP POLICY IF EXISTS "Users can view status history" ON feature_request_status_history;
DROP POLICY IF EXISTS "Admins can insert status history" ON feature_request_status_history;
...
-- Functions for vote counting (idempotent with CREATE OR REPLACE)
...
DROP TRIGGER IF EXISTS update_vote_count_on_vote_insert ON feature_request_votes;
DROP TRIGGER IF EXISTS update_vote_count_on_vote_update ON feature_request_votes;
DROP TRIGGER IF EXISTS update_vote_count_on_vote_delete ON feature_request_votes;
...
-- Function to update tag usage count (idempotent)
...
DROP TRIGGER IF EXISTS update_tag_usage_on_assignment ON feature_request_tag_assignments;
...
-- Insert default categories (idempotent with ON CONFLICT)
...
-- Insert default tags (idempotent with ON CONFLICT)
