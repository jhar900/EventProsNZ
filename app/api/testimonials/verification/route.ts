...
// GET /api/testimonials/verification - Check testimonial verification eligibility
export async function GET(request: NextRequest) {
  try {
    ...
    const validatedParams = verificationSchema.parse(queryParams);
    ...
    const { data: contractor, error: contractorError } = await supabase
      .from('users')
      .select('id, role, is_verified, first_name, last_name')
    ...
    return NextResponse.json({
      ...
      verification_data: {
        ...
        inquiry_id: validInquiry.id,
        inquiry_status: validInquiry.status,
        created_at: validInquiry.created_at,
      },
    });
  } catch (error) {
    ...
    console.error('Error in GET /api/testimonials/verification:', error);
    return NextResponse.json(
